...
class EmailChecker {
  constructor() {
    ...
    // FIXED: Correct email configuration structure
    this.emailConfigs = {
      gmail: [
        {
          user: process.env.GMAIL_USER1?.replace(/"/g, ""),
    ...
    Object.keys(this.emailConfigs).forEach((provider) => {
      ...
      configs.forEach((config, index) => {
        if (!config.user || !config.pass) {
          ...
          console.log(`⚠️ ${provider} Account ${index + 1}: Password might be too short`);
        } else {
  ...
  // FIXED: Get email config by email address
  ...
  // FIXED: Updated checkProvider to accept specific email config
      ...
      try {
        ...
        if (emails && emails.length > 0) {
          ...
          try {
            ...
            console.error(`⚠️ Error fetching email:`, fetchError.message);
            // Still mark as delivered since we found the email
            await this.updateTestResult(
              testId,
              provider,
  ...
  // FIXED: Updated updateTestResult to handle specific email addresses
  async updateTestResult(
    testId,
    provider,
    status,
  ...
  mapFolderName(folder, provider) {
    ...
    console.log(`🗂️ Folder mapping: ${folder} → ${mappedFolder}`);
    return mappedFolder;
  }
